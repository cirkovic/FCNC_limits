...
rm COMBINE_OUTPUT_medium.txt

for i in `ls cards`; do
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    echo $i >> COMBINE_OUTPUT_medium.txt
    cat ${FOLDER}/COMBINE_OUTPUT.txt >> COMBINE_OUTPUT_medium.txt
    echo >> COMBINE_OUTPUT_medium.txt;
    echo >> COMBINE_OUTPUT_medium.txt;
    echo >> COMBINE_OUTPUT_medium.txt;
done

grep "input_\|Expected \|Observed \|combine " COMBINE_OUTPUT_medium.txt > COMBINE_OUTPUT_medium_1.txt
mv COMBINE_OUTPUT_medium_1.txt COMBINE_OUTPUT_medium.txt
