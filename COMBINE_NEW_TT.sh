...
#        combine -S ${S} ..rds/${i} -M Asymptotic --rMin=-100 --rMax=100
    ...
    if [[ "1" == "0" ]]; then
        ...
        if [[ "this" == "not this" ]]; then
            ...
            '
            if   [[ "$i" == "input_MVAHutTT_b2j3_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHutTT_b3j3_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHutTT_b2j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHutTT_b3j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHutTT_b4j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHutTT_comb_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHctTT_b2j3_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHctTT_b3j3_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHctTT_b2j4_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHctTT_b3j4_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHctTT_b4j4_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind"
            elif [[ "$i" == "input_MVAHctTT_comb_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run blind --rMin=-0.1 --rMax=0.1"
            fi
            ':
            if   [[ "$i" == "input_MVAHutMAX_b2j3_hut.txt" ]]; then
