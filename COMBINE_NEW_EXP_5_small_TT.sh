...
#    if [[ "$i" != "input_MVAHct${LABEL}_comb_hct.txt" ]]; then continue; fi
    ...
    if [ "$CASE" == "split" ]; then
        if   [[ "$i" == "input_MVAHut${LABEL}_b2j3_hut.txt" ]]; then
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run expected --expectSignal 0.01 --rMin=-0.05 --rMax=0.05"
            ...
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run expected --expectSignal 0.01 --rMin=-0.01 --rMax=0.01"
