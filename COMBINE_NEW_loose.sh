    ...
    if [[ "1" == "0" ]]; then
        ...
        if [[ "this" == "not this" ]]; then
            ...
            if   [[ "$i" == "input_MVAHutMAX_b2j3_hut.txt" ]]; then
                ...
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
                #COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.5 --rMax=0.5"
                #COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.2 --rMax=0.2"
                ...
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.0005 --rMax=0.0005"
                #COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
                #COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.4 --rMax=0.4"
                ...
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.005 --rMax=0.005   --rAbsAcc=0.0001000000000000001  --rRelAcc=0.001000000000000001"
...
rm COMBINE_OUTPUT_loose.txt

for i in `ls cards`; do
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    echo $i >> COMBINE_OUTPUT_loose.txt
    cat ${FOLDER}/COMBINE_OUTPUT.txt >> COMBINE_OUTPUT_loose.txt
    echo >> COMBINE_OUTPUT_loose.txt;
    echo >> COMBINE_OUTPUT_loose.txt;
    echo >> COMBINE_OUTPUT_loose.txt;
done

grep "input_\|Expected \|Observed \|combine " COMBINE_OUTPUT_loose.txt > COMBINE_OUTPUT_loose_1.txt
mv COMBINE_OUTPUT_loose_1.txt COMBINE_OUTPUT_loose.txt
