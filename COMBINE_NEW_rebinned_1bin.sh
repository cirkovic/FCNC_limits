...
rm COMBINE_OUTPUT_rebinned_1bin.txt

for i in `ls cards`; do
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    echo $i >> COMBINE_OUTPUT_rebinned_1bin.txt
    cat ${FOLDER}/COMBINE_OUTPUT.txt >> COMBINE_OUTPUT_rebinned_1bin.txt
    echo >> COMBINE_OUTPUT_rebinned_1bin.txt;
    echo >> COMBINE_OUTPUT_rebinned_1bin.txt;
    echo >> COMBINE_OUTPUT_rebinned_1bin.txt;
done

grep "input_\|Expected \|Observed \|combine " COMBINE_OUTPUT_rebinned_1bin.txt > COMBINE_OUTPUT_rebinned_1bin_1.txt
mv COMBINE_OUTPUT_rebinned_1bin_1.txt COMBINE_OUTPUT_rebinned_1bin.txt
