...
for j in `ls cards/input_MVA2DComb_khut0p15__khct0p09_comb.txt`; do
#for j in `ls cards/input_MVA2DComb_khut0p09__khct0p15_comb.txt`; do
    i=`basename $j`
...
#    COMMAND="combine -S ${S} cards/${i} -M Asymptotic --minimizerAlgo=SeqMinimizer --rMin -5 --rMax 5 --minimizerTolerance 0.01 -n ${FOLDER}"
    COMMAND="combine -S ${S} cards/${i} -M Asymptotic -n ${FOLDER}"
    #COMMAND="combine -S ${S} cards/${i} -M Asymptotic -n ${FOLDER}"
