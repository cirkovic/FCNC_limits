...
#        #elif [[ "$i" == "input_MVAHut${LABEL}_comb_hut.txt" ]]; then COMMAND="combine -S ${S} ../cards/${i} -M MaxLikelihoodFit ${OPTIONSp} && combine -S ${S} ../cards/${i} -M Asymptotic ${OPTIONSp}"
...
#    fi
    if [ "$CASE" == "split" ]; then
        ...
        #elif [[ "$i" == "input_MVAHut${LABEL}_comb_hut.txt" ]]; then COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic ${OPTIONSp}"
