...
for fr in frlist: 
  ...
  for i in xrange(0, fr[1].correlationMatrix().GetNrows()):
    ...
    M.append(a)
  #print M
  h2 = TH2F("h2", "covariance matrix", fr[1].correlationMatrix().GetNrows(), -0.5, fr[1].correlationMatrix().GetNrows()-0.5, fr[1].correlationMatrix().GetNcols(), -0.5, fr[1].correlationMatrix().GetNcols()-0.5)
  print len(M)
