...
#        combine -S ${S} ..rds/${i} -M Asymptotic --rMin=-100 --rMax=100
    ...
    if [[ "1" == "0" ]]; then
    #if [[ "1" == "1" ]]; then
        COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic -D data_obs"
        ...
        if [[ "$i" == "input_MVAHctMAX_b3j4_hct.txt" ]]; then
            ...
            #ODAVDE
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic -D data_obs" # --stepSize=0.001 --minimizerStrategy 1"
        ...
        if [[ "this" == "not this" ]]; then
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=1 --strictBounds" # --stepSize=0.001 --minimizerStrategy 1"
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic -D data_obs" # --stepSize=0.001 --minimizerStrategy 1"
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic -D data_obsNew --nPoints 100 --qtilde=1" # --stepSize=0.001 --minimizerStrategy 1"
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic -D data_obs" # --stepSize=0.001 --minimizerStrategy 1"
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=1 --strictBounds" # --stepSize=0.001 --minimizerStrategy 1"
            #COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic -D data_obs" # --stepSize=0.001 --minimizerStrategy 1"
        else
