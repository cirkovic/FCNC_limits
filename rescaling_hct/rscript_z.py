...
processes = ['data_obs', 'sig_stop', 'sig_ttbar', 'sig', 'ttbb', 'ttcc', 'ttlf', 'other']
Processes = ['data_obs', 'SigStop', 'SigTTbar', 'Sig', 'ttbb', 'ttcc', 'ttlf', 'other']
...
for c in ['b2j3', 'b3j3', 'b2j4', 'b3j4', 'b4j4']:
    ...
    fs1a.append(TFile.Open('input/input_MVAHutComb_'+c+'_hut_Mergedbackgrounds.root'))
    fs2a.append(TFile.Open('input/input_MVAHctComb_'+c+'_hct_Mergedbackgrounds.root'))
    hs1 = {}
    hs2 = {}
    for pi, p in enumerate(processes):
        if p is not 'data_obs':
            ...
            for b in xrange(0, 20):
                if fs1a[-1].Get('combSTandTT_'+p+'_'+c+'StatBin'+Processes[pi]+str(b)+'Up') and fs2a[-1].Get('combSTandTT_'+p+'_'+c+'StatBin'+Processes[pi]+str(b)+'Up'):
                    S += ['_'+c+'StatBin'+Processes[pi]+str(b)]
            for s in S:
                if s is not '':
                    for v in ['Up', 'Down']:
