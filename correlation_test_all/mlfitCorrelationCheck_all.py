...
gStyle.SetOptStat(0)
#gStyle.SetTitleW(gStyle.GetTitleW()*0.1)
...
#  h2 = TH2F("h2", "correlation matrix", fr[1].correlationMatrix().GetNrows(), -0.5, fr[1].correlationMatrix().GetNrows()-0.5, fr[1].correlationMatrix().GetNcols(), -0.5, fr[1].correlationMatrix().GetNcols()-0.5)
  ...
  if "_comb_" in parsed[2]:
    tresf = 0.3/6
  else:
    tresf = 0.3
  #h2.SetMarkerSize(0.1*h2.GetMarkerSize())
  h2.SetMarkerSize(tresf*h2.GetMarkerSize())
  h2.GetXaxis().SetNdivisions(2, 0, 0, False)
  h2.GetXaxis().SetTickLength(0)
  #h2.GetXaxis().SetLabelSize(0.1*h2.GetXaxis().GetLabelSize())
  h2.GetXaxis().SetLabelSize(tresf*h2.GetXaxis().GetLabelSize())
  h2.GetYaxis().SetNdivisions(2, 0, 0, False)
  h2.GetYaxis().SetTickLength(0)
  #h2.GetYaxis().SetLabelSize(0.1*h2.GetYaxis().GetLabelSize())
  h2.GetYaxis().SetLabelSize(tresf*h2.GetYaxis().GetLabelSize())
  '''
  xnew = []
  for i in xrange(0, h2.GetXaxis().GetNbins()):
    if "StatBin" in h2.GetXaxis().GetBinLabel(i):
      continue
    else:
      xnew.append(h2.GetXaxis().GetBinLabel(i))
  print xnew
  ynew = []
  for i in xrange(0, h2.GetYaxis().GetNbins()):
    if "StatBin" in h2.GetYaxis().GetBinLabel(i):
      continue
    else:
      ynew.append(h2.GetYaxis().GetBinLabel(i))
  print ynew
  ...
  for i in xrange(0, h2r.GetYaxis().GetNbins()):
    h2r.GetYaxis().SetBinLabel(i+1, ynew[i])
  ...
  for i in xrange(0, h2.GetXaxis().GetNbins()):
    if h2.GetXaxis().GetBinLabel(i+1) == h2r.GetXaxis().GetBinLabel(i+1):
      for j in xrange(0, h2.GetYaxis().GetNbins()):
        if h2.GetYaxis().GetBinLabel(j+1) == h2r.GetYaxis().GetBinLabel(j+1):
          h2r.SetBinContent(i+1, j+1, h2.GetBinContent(i+1, j+1))
        else:
          j += 1
    else:
      i += 1
...
#  rfrt.setInitParListInh(floatParList)
  ...
  '''
  #c1 = TCanvas("c1", "complete correlation matrix", 20*800, 20*600)
  #c1 = TCanvas("c1", "", 30*800, 30*600)
  if "_comb_" in parsed[2]:
    #c1 = TCanvas("c1", "", 60*800, 60*600)
    c1 = TCanvas("c1", "", 10*800, 10*600)
  else:
    c1 = TCanvas("c1", "", 10*800, 10*600)
  #c1.SetLeftMargin(c1.GetLeftMargin()*1.6)
  h2.SetTitle("")
  h2.Draw("COL TEXT")
  c1.Print("correlation_matrix_"+parsed[1]+"_"+parsed[2]+".png")
  c1.Print("correlation_matrix_"+parsed[1]+"_"+parsed[2]+".pdf")
  print "********************************************"
  print "********************************************"
  print quality 
  if quality==-1: 
