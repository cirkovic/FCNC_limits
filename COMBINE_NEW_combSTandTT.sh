...
for j in `ls cards`; do
    i=$j
    #if [[ "$i" != "input_MVAHctComb_b4j4_hct.txt" ]]; then continue; fi
    #if [[ "$i" != "input_MVAHutComb_b4j4_hut.txt" ]]; then continue; fi
#    if [[ "$i" != "input_MVAHctComb_comb_hct.txt" ]]; then continue; fi
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    rm -rf ${FOLDER}
    mkdir ${FOLDER}
    ...
    #if [[ "$i" != "input_MVAHutComb_comb_hut.txt" ]] && [[ "$i" != "input_MVAHctComb_comb_hct.txt" ]]; then continue; fi
    #echo $i 2>&1 | tee COMBINE_OUTPUT.txt
    #combine -S ${S} ..rds/${i} -M Asymptotic  --rMin=-0.001 --rMax=0.001
    #combine -S ${S} ..rds/${i} -M Asymptotic #-S 0
#    if [[ "$i" != "input_MVAHctComb_comb_hct.txt" ]] && [[ "$i" != "input_MVAHutComb_comb_hut.txt" ]]; then continue; fi
#    if [[ "$i" == "input_MVAHutComb_b2j4_hut.txt" ]]; then
#        combine -S ${S} ..RDS/${i} -M Asymptotic --rMin=-5 --rMax=5
#    else
#    if [[ "$i" == "input_MVAHutComb_b3j4_hut.txt" ]]; then
...
#        combine -S ${S} ..rds/${i} -M Asymptotic --rMin=-100 --rMax=100
    ...
    if [[ "1" == "0" ]]; then
        ...
        if [[ "$i" == "input_MVAHctComb_b3j4_hct.txt" ]]; then
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-10 --rMax=10"
        elif [[ "$i" == "input_MVAHutComb_b2j4_hut.txt" ]]; then
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.001 --rMax=0.001"
        elif [[ "$i" == "input_MVAHutComb_b4j4_hut.txt" ]]; then
        ...
        elif [[ "$i" == "input_MVAHctComb_b4j4_hct.txt" ]]; then
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.0005 --rMax=0.0005"
        elif [[ "$i" == "input_MVAHctComb_comb_hct.txt" ]]; then
        ...
        elif [[ "$i" == "input_MVAHutComb_comb_hut.txt" ]]; then
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=1 --strictBounds" # --stepSize=0.001 --minimizerStrategy 1"
        else
            #if [[ "$i" == "input_MVAHctComb_b4j4_hct.txt" ]]; then
        ...
        if [[ "this" == "not this" ]]; then
            ...
            ':
            if   [[ "$i" == "input_MVAHutComb_b2j3_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHutComb_b3j3_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHutComb_b2j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.05 --rMax=0.05"
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            ...
            elif [[ "$i" == "input_MVAHutComb_b3j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHutComb_b4j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHutComb_comb_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.05 --rMax=0.05"
            elif [[ "$i" == "input_MVAHctComb_b2j3_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHctComb_b3j3_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHctComb_b2j4_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.05 --rMax=0.05"
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.5 --rMax=0.5"
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-100 --rMax=100"
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0045 --rMax=0.0045"
            elif [[ "$i" == "input_MVAHctComb_b3j4_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHctComb_b4j4_hct.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=-0.1 --rMax=0.1"
            elif [[ "$i" == "input_MVAHctComb_comb_hct.txt" ]]; then
...
rm COMBINE_OUTPUT_combSTandTT.txt

for i in `ls cards`; do
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    echo $i >> COMBINE_OUTPUT_combSTandTT.txt
    cat ${FOLDER}/COMBINE_OUTPUT.txt >> COMBINE_OUTPUT_combSTandTT.txt
    echo >> COMBINE_OUTPUT_combSTandTT.txt;
    echo >> COMBINE_OUTPUT_combSTandTT.txt;
    echo >> COMBINE_OUTPUT_combSTandTT.txt;
done

grep "input_\|Expected \|Observed \|combine " COMBINE_OUTPUT_combSTandTT.txt > COMBINE_OUTPUT_combSTandTT_1.txt
mv COMBINE_OUTPUT_combSTandTT_1.txt COMBINE_OUTPUT_combSTandTT.txt
