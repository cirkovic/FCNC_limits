...
#        combine -S ${S} ..rds/${i} -M Asymptotic --rMin=-100 --rMax=100
    ...
    if [[ "1" == "0" ]]; then
        ...
        if [[ "this" == "not this" ]]; then
            ...
            if   [[ "$i" == "input_MVAHutMAX_b2j3_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run expected --rMin=-1.0 --rMax=1.0"
            elif [[ "$i" == "input_MVAHutMAX_b3j3_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run expected --rMin=-1.0 --rMax=1.0"
            elif [[ "$i" == "input_MVAHutMAX_b2j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run expected --rMin=-1.0 --rMax=1.0"
                ...
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --run expected --rMin=-100 --rMax=100"
            fi
...
rm COMBINE_OUTPUT_rebinned_2bins.txt

for i in `ls cards`; do
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    echo $i >> COMBINE_OUTPUT_rebinned_2bins.txt
    cat ${FOLDER}/COMBINE_OUTPUT.txt >> COMBINE_OUTPUT_rebinned_2bins.txt
    echo >> COMBINE_OUTPUT_rebinned_2bins.txt;
    echo >> COMBINE_OUTPUT_rebinned_2bins.txt;
    echo >> COMBINE_OUTPUT_rebinned_2bins.txt;
done

grep "input_\|Expected \|Observed \|combine " COMBINE_OUTPUT_rebinned_2bins.txt > COMBINE_OUTPUT_rebinned_2bins_1.txt
mv COMBINE_OUTPUT_rebinned_2bins_1.txt COMBINE_OUTPUT_rebinned_2bins.txt
