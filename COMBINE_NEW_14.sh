...
#        combine -S ${S} ..rds/${i} -M Asymptotic --rMin=-100 --rMax=100
    ...
    if [[ "1" == "0" ]]; then
        ...
        if [[ "$i" == "input_MVAHctMAX_b3j4_hct.txt" ]]; then
            ...
            COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=100 --strictBounds" # --stepSize=0.001 --minimizerStrategy 1"
            #COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=10 --strictBounds" # --stepSize=0.001 --minimizerStrategy 1"
        ...
        if [[ "this" == "not this" ]]; then
            ...
            if   [[ "$i" == "input_MVAHutMAX_b2j3_hut.txt" ]]; then
                ...
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=5 --strictBounds"
            elif [[ "$i" == "input_MVAHutMAX_b2j4_hut.txt" ]]; then
                COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic --rMin=0.0001 --rMax=5 --strictBounds"
