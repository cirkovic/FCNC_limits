...
#OPTIONS="$OPTIONS --rMin=-3 --rMax=3"
################################################################################################################################################################## _1 OPTIONS="$OPTIONS --rMin=0.0 --rMax=50.0"
...
#    fi
#    if [[ "$CASE" == "split" ]]; then
#        #if   [[ "$i" == "input_MVAHut${LABEL}_b2j3_hut.txt" ]]; then COMMAND="combine -S ${S} ../cards/${i} -M MaxLikelihoodFit $OPTIONSp && combine -S ${S} ../cards/${i} -M Asymptotic $OPTIONSp"
...
#    fi
    if [[ "$CASE" == "split" ]]; then
        #if   [[ "$i" == "input_MVAHut${LABEL}_b2j3_hut.txt" ]]; then COMMAND="combine -S ${S} ../cards/${i} -M Asymptotic $OPTIONSp"
