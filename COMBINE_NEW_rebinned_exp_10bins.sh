...
rm COMBINE_OUTPUT_rebinned_10bins.txt

for i in `ls cards`; do
    FOLD=${i/input/INPUT}
    FOLDER=${FOLD/.txt/}
    echo $i >> COMBINE_OUTPUT_rebinned_10bins.txt
    cat ${FOLDER}/COMBINE_OUTPUT.txt >> COMBINE_OUTPUT_rebinned_10bins.txt
    echo >> COMBINE_OUTPUT_rebinned_10bins.txt;
    echo >> COMBINE_OUTPUT_rebinned_10bins.txt;
    echo >> COMBINE_OUTPUT_rebinned_10bins.txt;
done

grep "input_\|Expected \|Observed \|combine " COMBINE_OUTPUT_rebinned_10bins.txt > COMBINE_OUTPUT_rebinned_10bins_1.txt
mv COMBINE_OUTPUT_rebinned_10bins_1.txt COMBINE_OUTPUT_rebinned_10bins.txt
