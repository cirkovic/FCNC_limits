...
#OPTIONS="$OPTIONS --minos=all --minimizerAlgo=SeqMinimizer --minimizerAlgoForMinos=SeqMinimizer --minimizerToleranceForMinos 0.001"
OPTIONS="$OPTIONS --rMin=-1 --rMax=1 --stepSize=0.05 --minos=all --minimizerAlgo=SeqMinimizer --minimizerAlgoForMinos=SeqMinimizer --minimizerToleranceForMinos 0.001"
OPTIONSothr="$OPTIONSothr --rMin=-3 --rMax=3 --stepSize=0.05 --minos=all --minimizerAlgo=SeqMinimizer --minimizerAlgoForMinos=SeqMinimizer --minimizerToleranceForMinos 0.001"
#OPTIONS="$OPTIONS --minos --minimizerAlgoForMinos --minimizerStrategyForMinos=1 --minimizerToleranceForMinos 0.001"
...
#    PS="$PS $!"
    ...
    if [[ "$j" != *"b2j3"* ]]; then
        ...
        COMMAND="combine -S ${S} -M MaxLikelihoodFit ${OPTIONSothr} ../cards/${i} -n _1 && python /afs/cern.ch/work/c/cirkovic/kLimits/CombineHarvester/CMSSW_7_4_7/src/HiggsAnalysis/CombinedLimit/test/diffNuisances_noMCstat.py -a mlfit_1.root -g plots_1.root"
    fi
#    if [[ "$i" == *"b3j4_hut"* ]]; then
#        COMMAND="combine -S ${S} -M MaxLikelihoodFit ${OPTIONS} --expectSignal 0.04 ../cards/${i} -n _1 --minos=all && python /afs/cern.ch/work/c/cirkovic/kLimits/CombineHarvester/CMSSW_7_4_7/src/HiggsAnalysis/CombinedLimit/test/diffNuisances_noMCstat.py -a mlfit_1.root -g plots_1.root"
#    fi
...
#        COMMAND="combine -S ${S} -M MaxLikelihoodFit ${OPTIONS} ../cards/${i} --rMin=-3.0 --rMax=3.0 -n _1 && python /afs/cern.ch/work/c/cirkovic/kLimits/CombineHarvester/CMSSW_7_4_7/src/HiggsAnalysis/CombinedLimit/test/diffNuisances_noMCstat.py -a mlfit_1.root -g plots_1.root"
