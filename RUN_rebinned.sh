...
OUTPUT=~/www/22-02-2017/rebinned/${1}


unlink input
ln -s input_medium_rebinned/${1} input
...
cp -f produce_cards/rebinned/produceCards_${1}.C produceCards.C
./produceCards.zsh
cd cards
sh ../make_combos.sh
cd ..
...
python parse_combine_output.py COMBINE_OUTPUT_rebinned.txt
cd limits
#./run.sh $OUTPUT 45 30
#./run.sh $OUTPUT 25 25
#./run.sh $OUTPUT 40 40
...
rm -rf backup_${1}
mkdir backup_${1}
cp -r cards INPUT_* COMBINE_NEW_rebinned.sh produceCards.C backup_${1}
