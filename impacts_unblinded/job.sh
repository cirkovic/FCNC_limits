...
limit=`./../../getMeanExpected.sh ../../COMBINE_OUTPUT_combSTandTT_OBS.txt ${3}`
OPTIONS="$OPTIONS --expectSignal $limit"
...
COMMAND="combineTool.py -M Impacts -d ${wp} -m ${M} --doInitialFit            ${RMIN_RMAX} ${OPTIONS}"
echo $COMMAND
eval $COMMAND
echo
echo

COMMAND="combineTool.py -M Impacts -d ${wp} -m ${M} --doFits --parallel 4                ${RMIN_RMAX} ${OPTIONS}"
echo $COMMAND
eval $COMMAND
echo
echo

#COMMAND="combineTool.py -M Impacts -d common/${3} -m ${M} --doFits                          ${RMIN_RMAX} ${OPTIONS}"
#echo $COMMAND
#eval $COMMAND
#echo
#echo

COMMAND="combineTool.py -M Impacts -d ${wp} -m ${M} -o impacts_${name}.json            ${RMIN_RMAX} ${OPTIONS}"
echo $COMMAND
eval $COMMAND
echo
echo

#COMMAND="python /afs/cern.ch/work/c/cirkovic/kLimits/CombineHarvester/CMSSW_7_4_7/src/CombineHarvester/CombineTools/scripts/plotImpacts_1.py -i impacts_${name}.json -o impacts_${name}"
...
#COMMAND="python /afs/cern.ch/work/c/cirkovic/kLimits/CombineHarvester/CMSSW_7_4_7/src/CombineHarvester/CombineTools/scripts/plotImpacts_2.py -i impacts_${name}.json -o impacts_${name}"
#COMMAND="python /afs/cern.ch/work/c/cirkovic/kLimits/CombineHarvester/CMSSW_7_4_7/src/CombineHarvester/CombineTools/scripts/plotImpacts_3.py -i impacts_${name}.json -o impacts_${name}"
echo $COMMAND
eval $COMMAND
echo
echo
